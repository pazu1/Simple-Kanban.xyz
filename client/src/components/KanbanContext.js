...
class KanbanContextProvider extends React.Component {
  ...
  addCard(columnTitle, index) {
    this.setState((prevState) => {
      ...
      let copyColumn = copyColumns.find((col) => col.title === columnTitle);
  ...
  async removeCard(card) {
    ...
    let cardColumn = copyColumns.find((col) => col.title === card.column).cards;
  ...
  cancelCardEdit() {
    ...
    copyColumns.find((col) => col.title === cancelledCard.column).cards.pop();
    this.setState({
      unfinishedCard: null,
      columns: copyColumns,
    });
  ...
  async changeCardPosition(card, toColumn, toIndex = 0) {
    ...
    if (card.column === toColumn) {
      this.setState(
        (prevState) => {
          let copyColumns = prevState.columns;
          let array = copyColumns.find((col) => col.title === toColumn).cards;
    ...
    this.setState(
      (prevState) => {
        let copyColumns = prevState.columns;
        let toColumnObj = copyColumns.find((col) => col.title === toColumn);
        let oldColumnObj = copyColumns.find((col) => col.title === card.column);
  ...
  async updateCardPriority(priority, card) {
    ...
    let res = await this.API.updateCard(card.id, card.description, priority);
    this.setState({
      columns: copyColumns,
    });
  }
